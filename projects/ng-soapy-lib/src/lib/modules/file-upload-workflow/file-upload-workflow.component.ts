...
@Component({
  selector: 'lib-file-upload-workflow',
  templateUrl: './file-upload-workflow.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: forwardRef(() => FileUploadWorkFlowComponent)
    }
  ]
})
export class FileUploadWorkFlowComponent implements ControlValueAccessor {
  ...
  constructor(private fileUploadWorkFlowService: FileUploadWorkFlowService) {}
    ...
    if (this.selectedFiles.length > 0) {
      ...
      this.selectedFiles.forEach((selectedFile, index) => {
        ...
        if (index > this.uploadProgress.length - 1) {
          ...
          const task = this.fileUploadWorkFlowService.pushFileToStorage(
