...
import { FileUpload, FileUploaderValue } from './file-upload.model';
...
export class FileUploadComponent implements ControlValueAccessor {
    ...
    if (this.selectedFiles.length > 0) {
      ...
      this.selectedFiles.forEach((selectedFile, index) => {
        ...
        if (index > this.uploadProgress.length - 1) {
          ...
          const task = this.fileUploadService.pushFileToStorage(
  ...
  onFileChange($event: any) {
    if (this._fileSelector && this._fileSelector.files) {
      ...
      // for (let i = 0; i < this._fileSelector.files.length; i++) {
      //   //check if file has already been added
      //   if (
      //     !this.selectedFiles.some(
      //       (file) => this._fileSelector!.files![i].name === file.name
      //     )
      //   ) {
      //     this.selectedFiles.push(this._fileSelector.files[i]);
      //   }
      // }
      this.onUpload();
    }
