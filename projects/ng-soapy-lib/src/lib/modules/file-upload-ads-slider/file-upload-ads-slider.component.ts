...
@Component({
  selector: 'lib-file-upload-ads-slider',
  templateUrl: './file-upload-ads-slider.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: forwardRef(() => FileUploadAdsSliderComponent)
    }
  ]
})
export class FileUploadAdsSliderComponent implements ControlValueAccessor {
  ...
  constructor(private fileUploadAdsSliderService: FileUploadAdsSliderService) {}
    ...
    if (this.selectedFiles.length > 0) {
      ...
      this.selectedFiles.forEach((selectedFile, index) => {
        ...
        if (index > this.uploadProgress.length - 1) {
          ...
          const task = this.fileUploadAdsSliderService.pushFileToStorage(
